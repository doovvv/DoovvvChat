...
void HttpConnection::HandleReq() {
	...
	_response.keep_alive(false);//不需要保持长连接
	//处理get请求
	if (_request.method() == http::verb::get) {
		PreParseGetParam();
		bool success = LogicSystem::GetInstance()->HandleReq(_get_url, shared_from_this()); //处理get请求
		if (!success) {
...
void HttpConnection::WriteResponse() {
	...
	http::async_write(_socket, _response, [self](beast::error_code ec, std::size_t bytes_transferred) {
		try
		{
			self->_socket.shutdown(tcp::socket::shutdown_send, ec);//关闭发送端的socket
			self->_deadline.cancel(); //取消定时器，定时器的回调不会发生，socket就不会关闭
			if (ec) {
